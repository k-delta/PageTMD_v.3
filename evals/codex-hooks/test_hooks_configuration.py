...
SESSION_CONTEXT = ROOT / ".codex/hooks/load-session-context.py"
SUBAGENT_REPORT = ROOT / ".codex/hooks/validate-subagent-report.py"
...
class HookConfigurationTests(unittest.TestCase):
    ...
    def test_registers_exactly_the_five_designed_events(self) -> None:
        hooks = self.config["hooks"]
        self.assertEqual(
            set(hooks),
            {"PreToolUse", "PostToolUse", "SessionStart", "SubagentStop", "Stop"},
        )
    ...
    def test_session_start_registration_loads_focused_context(self) -> None:
        group = self.config["hooks"]["SessionStart"][0]
        ...
        self.assertEqual(
            ...
            "Loading focused repository context",
        )
        self.assertIn("git rev-parse --show-toplevel", handler["command"])
        self.assertIn(".codex/hooks/load-session-context.py", handler["command"])
        self.assertIn("python3", handler["command"])


    def test_subagent_stop_registration_targets_specialized_agents(self) -> None:
        group = self.config["hooks"]["SubagentStop"][0]
        self.assertEqual(
            group["matcher"],
            "^(architecture-reviewer|database-reviewer|performance-reviewer|security-reviewer|test-reviewer|test-runner)$",
        )
        self.assertEqual(len(group["hooks"]), 1)
        handler = group["hooks"][0]
        self.assertEqual(handler["type"], "command")
        ...
        self.assertEqual(
            handler["statusMessage"],
            "Checking specialized subagent report",
        )
        self.assertIn("git rev-parse --show-toplevel", handler["command"])
        self.assertIn(".codex/hooks/validate-subagent-report.py", handler["command"])
        self.assertIn("python3", handler["command"])
    ...
    def test_all_referenced_scripts_exist_and_stop_hook_is_executable(self) -> None:
        ...
        self.assertTrue(SUBAGENT_REPORT.is_file())
